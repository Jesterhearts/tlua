...
impl<'list, T> ListNode<'list, T> {
    ...
    pub fn next<'s>(&'s self) -> Option<&'s ListNode<'list, T>> {
        self.next.as_deref()
    }
}
...
impl<'c, 'list, T> CursorMut<'c, 'list, T> {
    ...
    pub fn insert_advance(self, next: &'list mut ListNode<'list, T>) -> Self {
...
impl<'list, T> List<'list, T> {
    pub fn cursor_mut<'c>(&'c mut self) -> CursorMut<'c, 'list, T> {
        CursorMut {
            len: &mut self.len,
    ...
    pub fn new(head: &'list mut ListNode<'list, T>) -> Self {
        Self {
            len: 1,
            head: Some(head),
        }
    ...
    pub fn head(&self) -> Option<&'_ ListNode<'list, T>> {
        self.head.as_deref()
    }

    pub fn from_slice(nodes: &'list mut [ListNode<'list, T>]) -> Self {
    ...
    pub fn iter<'a>(&'a self) -> Iter<'a, 'list, T> {
        Iter {
            remain: self.len,
            current: self.head(),
        }
