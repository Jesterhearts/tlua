            ...
            |(name, attribute)| {
                let attribute = match attribute {
                    ...
                    Some(attribute) => Some(match &*attribute {
                        ...
                        _ => {
                            return Err(SyntaxError::InvalidAttribute {
                                attrib: attribute.to_string(),
                            })
                        }
                    }),
                };
...
pub(crate) fn local_varlist1<'src, 'chunk>(
    mut input: Span<'src>,
...
mod tests {
    ...
    #[test]
    pub(crate) fn parses_local() -> anyhow::Result<()> {
        let local = "local foo";
    ...
    #[test]
    pub(crate) fn parses_local_namelist() -> anyhow::Result<()> {
    ...
    #[test]
    pub(crate) fn parses_local_with_attrib() -> anyhow::Result<()> {
    ...
    #[test]
    pub(crate) fn parses_local_init() -> anyhow::Result<()> {
