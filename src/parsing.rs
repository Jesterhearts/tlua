...
pub(crate) const LUA_WHITESPACE: &[u8] = b"\n\r\t\x0B\x0C ";

pub(crate) type Span<'a> = nom_locate::LocatedSpan<&'a [u8]>;
pub(crate) type InternalLuaParseError<'a> = nom_supreme::error::ErrorTree<Span<'a>>;
pub(crate) type ParseResult<'a, T> = IResult<Span<'a>, T, InternalLuaParseError<'a>>;
...
macro_rules! final_parser {
    ($input:expr => $parser:expr) => {
        ::nom_supreme::final_parser::final_parser($parser)($input).map_err(
            |e: crate::parsing::InternalLuaParseError| {
...
pub(crate) trait Parse<'chunk>: Sized {
    fn parse<'src>(input: Span<'src>, alloc: &'chunk ASTAllocator) -> ParseResult<'src, Self>;
}

pub(crate) fn lua_whitespace0(mut input: Span) -> ParseResult<()> {
    loop {
        input = match alt((
...
pub(crate) fn lua_whitespace1(input: Span) -> ParseResult<()> {
    preceded(
        alt((
...
pub(crate) fn parse_chunk<'src, 'chunk>(
    input: &'src str,
    alloc: &'chunk ASTAllocator,
) -> Result<Block<'chunk>, ErrorTree<LuaParseError>> {
    final_parser!(
...
pub(crate) fn is_keyword(span: Span) -> bool {
    matches!(
        *span,
        b"and"
            | b"break"
